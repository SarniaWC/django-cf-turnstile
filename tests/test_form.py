...
from django_cf_turnstile.fields import TurnstileCaptchaField, TEST_SITE_KEY, TEST_SECRET_KEY
...
class TestTurnstileCaptchaField(TestCase):
    ...
    def test_captcha_invalid_token(self, mock_post_form):
        ...
        mock_post_form.return_value = (200, {"success": False, "error-codes": ["invalid-input-response"]})
    ...
    def test_captcha_api_error_malformed_json(self, mock_post_form):
        ...
        mock_post_form.return_value = (200, {"error-codes": ["bad-request"]}) # 'success' key is missing
        form_data = {"cf-turnstile-response": "some-token"}
        form = self.TestForm(form_data)
        self.assertFalse(form.is_valid())
        self.assertIn("captcha", form.errors)
    ...
    @mock.patch.object(TurnstileCaptchaField, 'get_remote_ip', return_value="123.123.123.123")
    @mock.patch.object(client, 'post_form')
